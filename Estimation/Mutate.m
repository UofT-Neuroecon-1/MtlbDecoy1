...
if  strcmp(model,'Logit')
    ...
    for m = 1 : MSteps
        ...
        if all(propTheta.theta > 0) && propTheta.theta(1) <=1
            logPriorRatio = 2 * log (propTheta.theta(1)/particle.theta(1)) ...
                + sum(particle.theta(2:end) - propTheta.theta(2:end),2) / 2 ...
                + sum(log(propTheta.theta(2:end)./particle.theta(2:end)));
