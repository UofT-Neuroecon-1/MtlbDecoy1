function s = gevsf(x,k,sigma,mu)
%GEVSF Generalized extreme value survival function
%   S = GEVSF(X,K,SIGMA,MU) returns the survival function of the 
...
if nargin < 1
    error(message('gevsf:TooFewInputs'));
end
if nargin < 2, k = 0;     end
...
try
    s = 1 - gevcdf(x,k,sigma,mu);
catch
    error('gevsf:InputSizeMismatch',...
